...
The dataset we will be using today is from the [Centre for learning and life chances in knowledge economies and societies (LLAKES)](https://reshare.ukdataservice.ac.uk/851427/) study, which explores inequality in lifelong learning in the UK.^[There are a number of datasets that are built into R that I encourage you to play around with. They are very easy to download and can be found in the R [`datasets` package](https://stat.ethz.ch/R-manual/R-devel/library/datasets/html/00Index.html).] 

The data is saved as a `.sav` (SPSS) file, so we can use the `haven` package to import it. The `haven` package also allows you to import files in numerous formats, including `.csv`, `.dta`, and `.sas`. 

Below, we'll import the file 'perceptions_inequality.csv' using the `read.csv` function as an intuitive object called **inequality**. Make sure the data is stored in the working directory where you are running your R session/script.
